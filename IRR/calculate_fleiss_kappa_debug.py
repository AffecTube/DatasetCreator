...
def create_rating_matrix(fragments, labels):
...
def calculate_fleiss_kappa(matrix, n_annotators, label_to_index):
    """Calculate Fleiss' Kappa with debugging output."""
    ...
    for label, idx in label_to_index.items():
        print(f"{label}: {np.sum(matrix[:, idx])} annotator assignments")
...
def main(file_path="videos_annotations_devemo.json"):
    ...
    # Define annotators
    n_annotators = 3  # duygun, michal, szymon
    
    # Create rating matrix
    rating_matrix, label_to_index = create_rating_matrix(fragments, labels)
    
    # Calculate Fleiss' Kappa
    kappa = calculate_fleiss_kappa(rating_matrix, n_annotators, label_to_index)
